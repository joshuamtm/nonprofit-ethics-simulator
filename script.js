...
function loadScenario(index) {
    ...
    document.getElementById('ai-opportunity').textContent = scenario.aiOpportunity;
...
function showOutcome(pulledLever) {
    ...
    const outcomeText = pulledLever ? scenario.pullLeverOutcome : scenario.dontPullOutcome;
...
function showSummary() {
    ...
    userDecisions.forEach(decision => {
        const li = document.createElement('li');
        const decisionText = decision.pulledLever ? 'Implemented AI' : 'Kept Human Approach';
    ...
    return [
        {
            ...
            currentState: "Currently only able to award 50-60% of available funds annually due to manual review bottlenecks.",
            aiOpportunity: "Deploy an AI system to streamline grant application review and scoring.",
            benefits: [
            ...
            pullLeverOutcome: "You implemented the AI system. Funding increased to 98%, but 65% went to larger nonprofits with professional grant writers.",
            dontPullOutcome: "You kept human review. While only 60% of funds were distributed, grants went to a diverse mix of organizations.",
            reflection: "This highlights the tension between efficiency and equity in grantmaking."
        }
    ];
}